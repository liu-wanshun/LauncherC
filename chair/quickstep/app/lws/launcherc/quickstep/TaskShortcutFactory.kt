package app.lws.launcherc.quickstep
...
import app.lws.launcherc.quickstepcompat.eleven.WindowManagerCompatVR
...
object TaskShortcutFactory {
    ...
    class LegacySplitSystemShortcut(
        iconRes: Int, textRes: Int, activity: BaseDraggingActivity,
        taskContainer: TaskIdAttributeContainer, private val mLauncherEvent: LauncherEvent
        ...
        private fun makeLaunchOptions(activity: BaseDraggingActivity): ActivityOptions? {
            val navBarPosition: Int = WindowManagerCompatVR.getNavBarPosition(activity.getDisplayId())
            if (navBarPosition == WindowManagerCompatVR.NAV_BAR_POS_INVALID) {
                return null
            }
            val options = ActivityOptions.makeBasic()
            options.setLaunchWindowingMode(WindowManagerCompatVR.WINDOWING_MODE_SPLIT_SCREEN_PRIMARY)
            return options
        }
    ...
    @JvmField
    var LEGACY_SPLIT: TaskShortcutFactory = object : TaskShortcutFactory {
        ...
        private fun isAvailable(activity: BaseDraggingActivity, displayId: Int): Boolean {
            return if (Utilities.ATLEAST_T) {
                // android 13 use SPLIT_SELECT, not use this
                false
            } else (!activity.deviceProfile.isMultiWindowMode
